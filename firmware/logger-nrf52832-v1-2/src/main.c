...
int main(void){
    ...
    while(1){
        gpio_pin_toggle_dt(&led);
			...
			Measurement m;
			while (get_measurement(&rb, &m) == 0) {
				printf("Voltage: %d mV, Resistance: %d ohms, Power: %d nW\n", m.buffer_voltage_mv, m.resistance, m.pwr_nw);
				printf("Full?: %d\n", rb.is_full);
			}
        #endif

        #ifdef RINGBUFFER_UART_TX

			Measurement m;
			while (get_measurement(&rb, &m) == 0) {
				int32_t tx_buffer [] = {m.buffer_voltage_mv, m.resistance, m.pwr_nw};
	
				// printf("space?: %d\n", buffer_space(&rb));

				err = uart_tx(uart, tx_buffer, sizeof(tx_buffer), SYS_FOREVER_US);
				if (err) {	return err;	}
			}
